...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function play() {
    ...
    resultBox.innerHTML =
      '<p class="v-bold">Trial ' + (currentTrial + 1) + " in progress...</p>";
  ...
  function resetObservations() {
    ...
    trialRows.forEach(function (row) {
      row.querySelector(".cell-material").textContent = "-";
      row.querySelector(".cell-load").textContent = "-";
      row.querySelector(".cell-rhn").textContent = "-";
    });
    removeAverageRow();
    trialIndicator.textContent = "Trial 1 of 5";
    resultBox.innerHTML = '<p class="v-bold">Status: Ready</p>';
  }

  function updateTrialRow(index, rhnValue) {
    const row = trialRows[index];
    row.querySelector(".cell-material").textContent = materialName;
    row.querySelector(".cell-load").textContent = totalLoadKg.toString();
    row.querySelector(".cell-rhn").textContent = rhnValue.toString();
  }

  function appendAverageRow() {
    removeAverageRow();
    const average =
      trialValues.reduce(function (sum, value) {
        return sum + value;
      }, 0) / trialValues.length;
    const averageRow = document.createElement("tr");
    averageRow.id = "average-row";
    averageRow.innerHTML =
      '<td class="has-text-centered" colspan="4">Average Rockwell Hardness Number = ' +
      average.toFixed(1) +
      "</td>";
    document.getElementById("obs-body").appendChild(averageRow);
  }

  function removeAverageRow() {
    const existingAverageRow = document.getElementById("average-row");
  ...
  const bottomKnob = [
    ...
    [
      bottomStartx + bottomWidth3,
      bottomStarty + bottomLength1 + bottomLength2 + bottomLength3,
    ],
    [
      bottomStartx + (bottomWidth1 - bottomWidth3),
      bottomStarty + bottomLength1 + bottomLength2 + bottomLength3,
    ],
    [
      bottomStartx + (bottomWidth1 - bottomWidth3),
      bottomStarty + bottomLength1 + bottomLength2,
    ],
    [
      bottomStartx + (bottomWidth1 - bottomWidth2),
      bottomStarty + bottomLength1 + bottomLength2,
    ],
    [
      bottomStartx + (bottomWidth1 - bottomWidth2),
      bottomStarty + bottomLength1,
    ],
  ...
  function drawStatic(ctx, obj, color) {
    //for drawing the topKnob
    ctx.save();
    ctx.fillStyle = color;
    ctx.lineWidth = 1.5;
    ctx.beginPath();
  ...
  function draw() {
    ...
    if (reference[0][1] < 230) {
      ...
      zoom();
      pauseButton.setAttribute("disabled", "true");
      const rhnValue = rhnReadings[currentTrial];
      trialValues.push(rhnValue);
      updateTrialRow(currentTrial, rhnValue);
      currentTrial += 1;

      if (currentTrial < totalTrials) {
        trialIndicator.textContent = "Trial " + (currentTrial + 1) + " of 5";
        resultBox.innerHTML =
          '<p class="v-bold">Trial ' +
          currentTrial +
          " completed.</p><p>Press Play to perform Trial " +
          (currentTrial + 1) +
          ".</p>";
        playButton.removeAttribute("disabled");
      } else {
        appendAverageRow();
        trialIndicator.textContent = "Trial 5 of 5";
        resultBox.innerHTML =
          '<p class="v-bold">Experiment Completed Successfully.</p><p>Five Rockwell hardness readings have been recorded.</p>';
        playButton.setAttribute("disabled", "true");
      }
    }
  }
